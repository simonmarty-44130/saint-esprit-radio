...
class SaintEspritV3 {
    constructor() {
        ...
        this.multitrackEditor = null; // Éditeur multipiste
    ...
    switchView(viewName) {
        ...
        if (targetView) {
            ...
            if (viewName === 'news') {
                ...
                this.initOnAirMode();
            } else if (viewName === 'multitrack') {
                this.initMultitrackEditor();
            }
        }
    }
            ...
            <tr onclick="app.${editFunction}('${news.id}')">
                ...
                <td>
                    <div class="date-info">
                        ...
                        <span class="date-secondary">${news.site || createdDate.relative}</span>
                    </div>
                </td>
                <td>
                    <div class="date-info">
            ...
            <div class="editor-form">
                ...
                <div class="form-row form-row-3">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="news-site" value="${this.currentNews.site || ''}" placeholder="Paris, Lyon, Marseille...">
                    </div>
                </div>
    ...
    updateDurations() {
        ...
        // Seul le contenu compte pour le temps de lecture (lancement et pied sont des notes pour le présentateur)
        const content = contentEl.value || '';
        const lancement = document.getElementById('news-lancement')?.value || '';
        const pied = document.getElementById('news-pied')?.value || '';

        const durations = this.durationManager.calculateTotalDuration(content);
        ...
        editor.innerHTML = `
            <div class="editor-form">
                ...
                <div class="form-row form-row-4">
                    ...
                    <div class="form-group">
                        <label>Site/Ville</label>
                        <input type="text" id="animation-site" value="${this.currentAnimation.site || ''}" placeholder="Paris, Lyon...">
                    </div>
    ...
    async updateOnAirDisplay() {
        ...
        if (currentSegment.type === 'news') {
            ...
            if (news) {
                ...
                // Afficher lancement
                console.log('🔍 Test lancement:', news.lancement ? 'présent' : 'absent', lancementSection ? 'section trouvée' : 'section NON trouvée');
                if (news.lancement && news.lancement.trim()) {
                    console.log('✅ Affichage du lancement');
                    if (lancementSection) lancementSection.style.display = 'block';
                    if (lancementText) lancementText.innerHTML = news.lancement.replace(/\n/g, '<br>');
                } else {
                    console.log('❌ Pas de lancement à afficher');
                ...
                console.log('📋 Catégorie de la news:', news.category);
                ...
                console.log('🔍 Test pied:', news.pied ? 'présent' : 'absent', piedSection ? 'section trouvée' : 'section NON trouvée');
                if (news.pied && news.pied.trim()) {
                    console.log('✅ Affichage du pied');
                    if (piedSection) piedSection.style.display = 'block';
                    if (piedText) piedText.innerHTML = news.pied.replace(/\n/g, '<br>');
                } else {
                    console.log('❌ Pas de pied à afficher');
...
class AudioEditorV3 {
    ...
    async exportToNews() {
        ...
        try {
            ...
            // Update UI with file info first
            if (window.app.handleAudioUpload) {
                await window.app.handleAudioUpload(file);
            }
            
            // Upload to S3
            showNotification('Uploading MP3 to S3...', 'info');
            ...
            // Update current news with S3 URL
            ...
            // Reload the news editor to show updated info from S3
            if (window.app.currentNews) {
                await window.app.editNews(window.app.currentNews.id);
            }
            
            showNotification('Audio exported to news as MP3 (192kbps)!', 'success');
