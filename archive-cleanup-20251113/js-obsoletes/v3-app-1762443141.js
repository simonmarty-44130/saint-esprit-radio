...
class SaintEspritV3 {
    ...
    startOnAirTimer() {
        ...
        this.onAirTimerInterval = setInterval(() => {
            if (!this.onAirPaused && this.currentConductor) {
                ...
                const elapsed = Math.floor((Date.now() - this.onAirStartTime) / 1000);
    ...
    updateOnAirStats() {
        ...
        for (let i = 0; i < this.onAirCurrentIndex; i++) {
            elapsedSeconds += segments[i].duration || 0;
        }
        elapsedSeconds += Math.floor((Date.now() - this.onAirStartTime) / 1000);
