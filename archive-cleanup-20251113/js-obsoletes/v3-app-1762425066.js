...
class SaintEspritV3 {
    ...
    async loadJournals() {
        try {
            ...
            const journals = await window.StorageDynamoDB.getAllJournals();
        ...
        editor.innerHTML = `
            <div class="editor-content">
                ...
                <div class="news-picker">
                    <label>➕ Ajouter une news</label>
                    <select onchange="app.addNewsToJournal(this.value); this.value=''">
                        <option value="">Sélectionner une news...</option>
                        ${this.allNews.filter(n => n.duration > 0).map(n => `
                            <option value="${n.id}">${n.title} (${this.formatDurationMMSS(n.duration)})</option>
                        `).join('')}
                    </select>
                </div>
        ...
        list.innerHTML = newsInJournal.map((news, index) => `
            <div class="journal-news-item" data-index="${index}" draggable="true">
                ...
                <div class="news-item-content">
                    ...
                    <div class="news-item-meta">${news.type || 'news'} • ${new Date(news.createdAt).toLocaleDateString('fr-FR')}</div>
                </div>
    ...
    async saveJournal() {
        ...
        try {
            ...
            await window.StorageDynamoDB.saveJournal(this.currentJournal);
    ...
    async deleteJournal() {
        ...
        try {
            await window.StorageDynamoDB.deleteJournal(this.currentJournal.id);
    ...
    async generateConductorFromJournal() {
        ...
        await window.StorageDynamoDB.saveConductor(newConductor);
